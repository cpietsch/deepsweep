...
d3.json("data/eu.topojson", function(error, world) {
  var topo = topojson.feature(world, world.objects.europe);
  console.time("draw");
  // console.log(path(topojson.feature(world, world.objects.europe)));
  path(topo);
  context.stroke();
    console.timeEnd("draw");

  //canvas.datum(topojson.mesh(topojson.feature(world, world.objects.europe).features))
  //render()
  ...
  //     .datum(topojson.feature(world, {type: "GeometryCollection", geometries: world.objects.europe.geometries/*.filter(function(d) { return d.id == 840 || d.id == 124; })*/}))
  //     .attr("class", "countries")
  //     .attr("d", path);
